...
public class OIDCOauthManager {
    ...
    private JSONObject getClientCredentials() throws OIDCProviderException {
        ...
        try {
            ...
            StringBuilder builder = responseStringBuilder(response);
    ...
    public boolean validateToken(String token) throws OIDCProviderException {
        try {
            HttpResponse response = makeRequestforTokenValidity(token);
            StringBuilder builder = responseStringBuilder(response);
            JSONObject jsonObject = new JSONObject(builder.toString());
            log.info(jsonObject);
            if (response.getStatusLine().getStatusCode() != 200) {
                log.error("Failed to connect to Introspect endpoint in Identity Provider server." +
                        " Exited with Status Code " + response.getStatusLine().getStatusCode());
                return false;
     ...
     */
    private HttpResponse makeRequestforTokenValidity(String token) throws OIDCProviderException {
        try {
            HttpClient client = Utils.getAllSSLClient();
     ...
     */
    private StringBuilder responseStringBuilder(HttpResponse response) throws IOException {
